...
Next, I will fit univariate linear models to assess the p values of variables for consideration for inclusion into my model, at a level of statistical significance of p < 0.05.  A priori based on my exploratory ggplots, I will definitely select babysex, mrace, momwt, bhead, blength, gaweeks.
```{r}
...
Based on my univariate analyses of all covariates, the following variables are statisticall significant at the p < 0.05 level: babysex, mrace, momwt, gaweeks, fincome, frace, ppbmi, ppwt, smoken, and wtgain.  Bhead and blength have p values equal to zero - I wonder if these are collinear variables (since they all kind of measure the same thing).  I will include these anyway because, why not?
...
```{r}
#lm_3 = lm(bwt ~ gaweeks + bhead + blenth + momwt + babysex + mrace + )
```


Next, I will fit the two specified models and perform cross-validation just for these two models
```{r}
