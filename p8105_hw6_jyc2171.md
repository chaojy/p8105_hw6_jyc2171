...
spend some time building the models = fit some models, look for
statistical significant, look at residuals, etc.
...
Try to use the bootstrap to find distributions download dataset fit
model to dataset

fit a linear model of max temp vs min temp – two things interested in R2
value and estimated log(intercept \* estimated coefficient - get broom
tidy, get your estimated coefficients, get them next to each other, them
multiply and do log. start with one model first to get the process for
one, then how do you do it for multiple)
